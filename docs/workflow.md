...
`EventCycle.board_approved_at` is the gate and `WorkItem.board_released_at`
records the release, which also queues the department's email in the same
transaction. Delivery is not recorded on the work item; see
`docs/email-outbox.md` for where it lives. The detail page shows a held item
as `PENDING_BOARD_APPROVAL`, a display-only status like `UNDER_REVIEW` below.
